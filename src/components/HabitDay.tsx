...
export function HabitDay({ defaultCompleted = 0, amount = 0, date }: habitDayProps) {
  ...
  function handleAmountChange(completed: number){
    setCompleted(completed)
  }


  const dayAndMoon = dayjs(date).format("DD/MM");
  ...
  return (
    ...
    <Popover.Portal>
      <Popover.Content className="min-w-[320px] p-6 rounded-2xl bg-zinc-900 flex flex-col">
        ...
        <span className="mt-1 font-extrabold leading-tight text-3xl">{dayAndMoon}</span>

        <ProgressBar progress={comlpetedPercentage} />

        <HabitsList date={date} onCompletedChanged={handleAmountChange} />
